...
exec > >(tee -a /tmp/tvspro_debug.txt) 2>&1
set -x
...
wget --no-check-certificate 'https://github.com/Belfagor2005/revolutionpro/archive/refs/heads/main.tar.gz'
tar -xzf main.tar.gz
cp -r 'revolutionpro-main/usr' '/'
set +e
cd
sleep 2
...
IMAGE VERSION: $distro_version" >> /tmp/tvspro_debug.txt
sleep 5
killall -9 enigma2
exit 0