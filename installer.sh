...
TMPPATH=/tmp/revolutionpro-install
FILEPATH=/tmp/revolutionpro-main.tar.gz

echo "Starting revolutionpro installation..."

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/tvspro
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/tvspro
fi
...
cleanup() {
    ...
    [ -d "/tmp/revolutionpro-main" ] && rm -rf "/tmp/revolutionpro-main"
}
...
echo "⬇️ Downloading revolutionpro..."
wget --no-check-certificate 'https://github.com/Belfagor2005/revolutionpro/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "❌ Failed to download revolutionpro package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to extract revolutionpro package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/revolutionpro-main/usr/lib/enigma2/python/Plugins/Extensions/tvspro" ]; then
    cp -r "$TMPPATH/revolutionpro-main/usr/lib/enigma2/python/Plugins/Extensions/tvspro"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/revolutionpro-main/usr/lib64/enigma2/python/Plugins/Extensions/tvspro" ]; then
    cp -r "$TMPPATH/revolutionpro-main/usr/lib64/enigma2/python/Plugins/Extensions/tvspro"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/revolutionpro-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/revolutionpro-main/usr"/* /usr/ 2>/dev/null
